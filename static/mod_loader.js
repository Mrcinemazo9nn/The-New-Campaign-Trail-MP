...
$("#submitMod").click(function() {
    ...
    let selectedWidget = document.querySelector('.widget.selected_widget');
    if (!selectedWidget) {
        selectedWidget = document.querySelector('.widget');
    }
    if (selectedWidget) {
        const widgetVal = (selectedWidget.getAttribute('mod-value') || '').trim();
        if (widgetVal) {
    ...
    if ($("#modSelect")[0].value == "other") {
        ...
        var modVal = ($("#modSelect")[0].value || "").trim();
